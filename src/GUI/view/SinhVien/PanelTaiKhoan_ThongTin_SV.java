...
public class PanelTaiKhoan_ThongTin_SV extends JPanel {
	...
	public JTextField textField_NghanhHoc;
	public JTextField textField_Lop;
	public JTextField textField_SoDienThoai;

	public PanelTaiKhoan_ThongTin_SV() {
...
//      jTextField_HoVaTen.addActionListener(new SuKienThoiGianThi(this));
		...
		lblMaSinhVien.setHorizontalAlignment(SwingConstants.RIGHT);
		lblMaSinhVien.setFont(new Font("Arial", Font.BOLD, 16));
		lblMaSinhVien.setBounds(493, 83, 105, 28);
		jPanelBody.add(lblMaSinhVien);
		
		textField_MaSinhVien = new JTextField();
		textField_MaSinhVien.setFont(new Font("Arial", Font.BOLD, 14));
		textField_MaSinhVien.setBounds(651, 83, 229, 28);
		textField_MaSinhVien.setEnabled(false);
		jPanelBody.add(textField_MaSinhVien);
		
		JLabel lblNghanhHoc = new JLabel("Ngành học:");
		lblNghanhHoc.setHorizontalAlignment(SwingConstants.RIGHT);
		lblNghanhHoc.setFont(new Font("Arial", Font.BOLD, 16));
		lblNghanhHoc.setBounds(493, 127, 105, 28);
		jPanelBody.add(lblNghanhHoc);
		
		textField_NghanhHoc = new JTextField();
		textField_NghanhHoc.setFont(new Font("Arial", Font.BOLD, 14));
		textField_NghanhHoc.setBounds(651, 127, 229, 28);
		textField_NghanhHoc.setEnabled(false);
		jPanelBody.add(textField_NghanhHoc);
		
		JLabel lblLop = new JLabel("Lớp:");
		lblLop.setHorizontalAlignment(SwingConstants.RIGHT);
		lblLop.setFont(new Font("Arial", Font.BOLD, 16));
		lblLop.setBounds(493, 171, 105, 28);
		jPanelBody.add(lblLop);
		
		textField_Lop = new JTextField();
		textField_Lop.setFont(new Font("Arial", Font.PLAIN, 14));
		textField_Lop.setBounds(651, 171, 229, 28);
		textField_Lop.setEnabled(false);
		jPanelBody.add(textField_Lop);
	...
	private void DocDuLieu() {
		jTextField_HoVaTen.setText(SinhVienBLL.getInstance().getSinhVien(ManHinhChinh_SV.taiKhoanSV).getHoTen());
		if (SinhVienBLL.getInstance().getSinhVien(ManHinhChinh_SV.taiKhoanSV).getGioiTinh() == true) {
			jTextField_GioiTinh.setText("Nam");
		} else
			jTextField_GioiTinh.setText("Nữ");
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
		Date dob = SinhVienBLL.getInstance().getSinhVien(ManHinhChinh_SV.taiKhoanSV).getNgaySinh();
		jTextField_NgaySinh.setText(sdf.format(dob));
		textField_Lop.setText(SinhVienBLL.getInstance().getSinhVien(ManHinhChinh_SV.taiKhoanSV).getLop());
		textField_MaSinhVien.setText(SinhVienBLL.getInstance().getSinhVien(ManHinhChinh_SV.taiKhoanSV).getIDSv() + "");
		textField_NghanhHoc.setText(SinhVienBLL.getInstance().getSinhVien(ManHinhChinh_SV.taiKhoanSV).getNganhHoc());
		textField_SoDienThoai.setText(SinhVienBLL.getInstance().getSinhVien(ManHinhChinh_SV.taiKhoanSV).getSoDienThoai());
		NoiSinh.setSelectedItem(SinhVienBLL.getInstance().getSinhVien(ManHinhChinh_SV.taiKhoanSV).getNoiSinh());
	}
	
	
	private void luuDuLieu() {
		nutpanelLuu.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(textField_SoDienThoai.getText().replaceAll(" ", "").equals("") == true) {
					...
					SinhVienBLL.getInstance().UpdateThongTinSV(ManHinhChinh_SV.taiKhoanSV, NoiSinh.getSelectedItem().toString(), textField_SoDienThoai.getText());
